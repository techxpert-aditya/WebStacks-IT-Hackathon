...
ConnectDB();   $Tquery="select * from form_table where userid='$sesop'";
...
<table style="background:white;color:black" class="table btn-sm btn-info" id="table">
  ...
  <?php  
    ...
    echo '<td><BUTTON class="btn-sm btn-info btnget" retid='.$row['FORM_ID'].'>Edit <i class="fa fa-edit"></i></BUTTON></td>';
    if ($row["ISLOCKED"]=="1") {
      if(isset($_COOKIE['formidis'.$row['FORM_ID']]))
      {
        echo  '<td><BUTTON class="btn-sm btn-warning btnunlock" retid='.$row['FORM_ID'].'>UNLOCK</BUTTON></td>';
      }
      else
      {
    ...
    {
    echo  '<td><BUTTON class="btn-sm btn-warning btnlock" retid='.$row['FORM_ID'].'>LOCK</BUTTON></td>';
    }
    if ($row["ISLOCKED"]=="1") {
      if(isset($_COOKIE['formidis'.$row['FORM_ID']]))
      {
       echo  '<td><BUTTON class="btn-sm btn-danger btndel" delid='.$row['FORM_ID'].'>DELETE</BUTTON></td>';
      }
      else
      {
  ...
  <div class="modal-dialog" role="document">
    <div style="background:#061826;"class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style="color:white;">FORM CONFIGURATION</h5>
      ...
      <div class="modal-body">
        <label style="color:white;font-size:15px;" >ENTER NAME OF FORM PAGE :</label>
...
<script>
   $(document).ready(function(){ 
      ...
      $("#submitbtn").click(function (e) { 
        ...
        e.preventDefault();
        parent.window.location.href="http://localhost/WebStack/Modules/FormBuilder/index.php?id="+ e.target.getAttribute("retid");
      });
     function validate()
     {
