...
ConnectDB();   $Tquery="select * from editor_table where userid='$sesop'";
...
<table style="background:white;color:black" class="table btn-sm btn-info" id="table">
  ...
  <?php  
    ...
    echo '<td><BUTTON class="btn-sm btn-info btnget" retid='.$row['CODE_ID'].'>Edit <i class="fa fa-edit"></i></BUTTON></td>';
    if ($row["ISLOCKED"]=="1") {
      if(isset($_COOKIE['pageid'.$row['CODE_ID']]))
      {
        echo  '<td><BUTTON class="btn-sm btn-warning btnunlock" retid='.$row['CODE_ID'].'>UNLOCK</BUTTON></td>';
      }
      else
      {
    ...
    {
    echo  '<td><BUTTON class="btn-sm btn-warning btnlock" retid='.$row['CODE_ID'].'>LOCK</BUTTON></td>';
    }
    if ($row["ISLOCKED"]=="1") {
      if(isset($_COOKIE['pageid'.$row['CODE_ID']]))
      {
      echo  '<td><BUTTON class="btn-sm btn-danger btndel" delid='.$row['CODE_ID'].'>DELETE</BUTTON></td>';
      }
      else
      {
  ...
  <div class="modal-dialog" role="document">
    <div style="background:#061826;"class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style="color:white;">EDITOR FILE CONFIGURATION</h5>
...
<script>
   $(document).ready(function(){ 
      ...
      $("#submitbtn").click(function (e) { 
        ...
        e.preventDefault();
        parent.window.location.href="http://localhost/WebStack/Modules/CodeCompiler/index.php?id="+ e.target.getAttribute("retid");
      });
     function validate()
     {
